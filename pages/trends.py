...
def layout():
    ...
    return html.Div([
        html.Div(
            html.Div(
                [
                    # ── Hero (matches design TrendsTab) ───────────────────────
                    html.Div("Trends", className="dca-hero-eyebrow"),
                    html.H1("Distress trends", className="dca-hero-h1"),
                    html.P(
                        "Filing activity over the last 12 weeks, broken out by stage and county.",
                        className="dca-hero-lead",
                        style={"marginBottom": "22px"},
                    ),

                    # County filter (right-aligned alongside hero — not in design but useful)
                    html.Div(
                        dcc.Dropdown(
                    ...
                    # ── Row 1: Filings by stage + Top counties (matches design) ─
                    html.Div(
                        [
                            html.Div([
                    ...
                    # ── Row 2: 12-week filing volume (matches design) ──────────
                    html.Div([
                    ...
                    # ── Row 3: Monthly filings (full history) ─────────────────
                    html.Div([
...
def update_charts(counties):
    ...
    # ── Filings by stage (custom HTML bars — matches design) ──────────────────
    ...
    # ── Top counties (custom HTML bars — matches design) ──────────────────────
    by_county = (df.groupby("County", observed=True).size()
                   .reset_index(name="n")
                   .sort_values("n", ascending=False).head(8))
    max_c = max(by_county["n"].max() if len(by_county) else 1, 1)
    county_bar_items = []
    ...
    # ── 12-week filing volume — paired NOD/NTS bars (matches design chart style) ─
    cutoff = df["Recording Date"].max() - pd.Timedelta(weeks=12)
    weekly_raw = (
        df[df["Recording Date"] >= cutoff]
        .groupby(["Week", "Stage Short"], observed=True)
        .size().reset_index(name="Count")
    )
    # Pivot to get NOD and NTS side by side
    nod_weeks = weekly_raw[weekly_raw["Stage Short"] == "NOD"].set_index("Week")["Count"]
    nts_weeks = weekly_raw[weekly_raw["Stage Short"] == "NTS"].set_index("Week")["Count"]
    all_weeks  = sorted(weekly_raw["Week"].unique())
    nod_vals   = [int(nod_weeks.get(w, 0)) for w in all_weeks]
    nts_vals   = [int(nts_weeks.get(w, 0)) for w in all_weeks]
    week_labels = [w.strftime("W%-U") if hasattr(w, "strftime") else str(w) for w in all_weeks]

    fig_weekly = go.Figure([
        go.Bar(name="NOD", x=week_labels, y=nod_vals,
               marker_color="#D97706", marker_line_width=0),
        go.Bar(name="NTS", x=week_labels, y=nts_vals,
               marker_color="#DC2626", marker_line_width=0),
    ])
    fig_weekly.update_layout(
        **_plotly_layout(bargap=0.25, bargroupgap=0.05, barmode="group"),
        showlegend=True,
    )
    ...
    no_data = [html.P("No data", style={"color": "var(--ink-3)", "padding": "8px 0"})]
    return (
        stage_bar_items  or no_data,
        county_bar_items or no_data,
        fig_weekly, fig_monthly, fig_density,
    )
